...
from Downstream.MvGCN import MvGCN
from Utils.DataLoader import generate_permutation
...
from Utils.utils import accuracy, f1_test
from tqdm import tqdm

from args import args

from GuiMvdiff.Eva.tsne import draw_plt
...
def Classifier(adj, features, labels, nfeats, num_view, num_class, args, device):
    ...
    # features[{}] = torch.Tensor(features[{}] / 1.0).to(device)
    for i in range(num_view):
        # exec("features_{}= torch.from_numpy(features[{}]/1.0).float().to(device)".format(i, i))
        exec("features[{}]= torch.Tensor(features[{}] / 1.0).cuda()".format(i, i))
        exec("features[{}] = F.normalize(features[{}])".format(i, i))
        exec("adj[{}]=adj[{}].to_dense().float().cuda()".format(i, i))
    ...
    return out_model, features, labels, adj, idx_test, output,acc_test.item(),f1
...
def test(args, model, features, labels, adj, idx_test):
    ...
    draw_plt(args.dataset, output.detach().cpu().numpy(), labels.detach().cpu().numpy())
    print("Dataset:", args.dataset)
    print("Test set results:",
          "accuracy= {:.2f}".format(100 * acc_test.item()),
          "f1= {:.2f}".format(100 * f1))
    return 100 * acc_test.item(), 100 * f1
