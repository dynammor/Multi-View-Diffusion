    ...
    for key in obj.keys():
        ...
        if isinstance(item, h5py.Dataset):
            ...
            if isinstance(out[key], np.ndarray) and out[key].ndim == 2:
                out[key] = out[key].T  # 转置适应 MATLAB 存储方式
        elif isinstance(item, h5py.Group):
            out[key] = h5py_to_dict(item)  # 递归进入子 group
    return out
...
def LoadMatData(datasets, k, path="D:/Lab/dataset/multiview datasets/"):
    ...
    if datasets=='AwA' or datasets=='YTF20' or datasets=='YTF50' or datasets=='YTF100' or datasets=='YTF200':
        # data = load_mat_v73(f"{path}{datasets}.mat")
        data = hdf5storage.loadmat(path+datasets)
    else:
    ...
    for i in range(x.shape[1]):
        ...
        As.append(sparse_mx_to_torch_sparse_tensor(temp+ sp.eye(temp.shape[0])))
    ...
    return adj, features, labels, nfeats, len(nfeats), num_class,adj_noself,As
